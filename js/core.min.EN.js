...
$(document).ready(function () {
    var fade = 150, delay = 500, showmap = false, autobusca = true, minlength = 3;
    ...
    $("#datado, #datado-nota, #datado-recibo").show(function() {
        $("#datado, #datado-nota, #datado-recibo").datepicker({
            format: 'dd/mm/yyyy',
            todayHighlight: true,
            autoclose: true
        });
    ...
    $("#cliente, #cliente-recibo").show(function() {
        //$("#cliente").select2();
        $("#cliente, #cliente-recibo").autocomplete({
            url:"loadCliente.php",
    ...
    $(".load-toggle").on("click", function () {
        $(".page-load").fadeIn(fade);

        //$(this).ready(function () {
        $(window).load(function() {
            $(".page-load").fadeOut(fade);
        });
    });

    $(".page-load-close").on("click", function () {
        $(".page-load").fadeOut(fade);
    });

    $(".load-bkp").on("click", function () {
        $(".page-load").fadeIn(fade);
    ...
    $(".page-search-close").on("click", function () {
        $(".page-search").fadeOut(fade);
        $("#search-keyword-saude").val("");
        $("#search-result").empty();
    });

    $("#search-keyword-saude").keyup(function () {
        var that = this, value = $(this).val();

        if (value.length >= minlength) {
            $.ajax({
                type: "GET",
                url: "searchOcorrenciaSaude.php",
                data: {
                    'search_keyword' : value
                },
                dataType: "text",
    ...
    $(".page-search-close").on("click", function () {
        $(".page-search").fadeOut(fade);
        $("#search-keyword-secretaria").val("");
        $("#search-result").empty();
    });

    $("#search-keyword-secretaria").keyup(function () {
        var that = this, value = $(this).val();

        if (value.length >= minlength) {
            $.ajax({
                type: "GET",
                url: "searchOcorrenciaSecretaria.php",
                data: {
                    'search_keyword' : value
                },
                dataType: "text",
        ...
        if ($("#search-keyword-secretaria").val() === "") {
            $("#search-result").empty();
        }
    });
    ...
    $(".form-novo-tecnico").submit(function (e) {
        e.preventDefault();

	    $.post("insertTecnico.php", { nome: $("#nome").val(), nome: $("#nome-head").val(), rand: Math.random()}, function (data) {
            $(".btn-submit-novo-tecnico").html('<img src="img/rings.svg" class="loader-svg">').fadeTo(fade, 1);

            switch (data) {
            case 'true':
                $.smkAlert({text: 'T&eacute;cnico cadastrado com sucesso.', type: 'success', time: 2});
                //window.setTimeout("location.href='autor'", delay);
                $(".form-novo-tecnico")[0].reset();
                break;
            ...
            $(".btn-submit-novo-tecnico").html('Salvar').fadeTo(fade, 1);
        });

	    return false;
    });

    $(".form-novo-tecnico-head").submit(function (e) {
        e.preventDefault();

	    $.post("insertTecnico.php", { nome: $("#nome-head").val(), rand: Math.random()}, function (data) {
            $(".btn-submit-novo-tecnico-head").html('<img src="img/rings.svg" class="loader-svg">').fadeTo(fade, 1);

            switch (data) {
            case 'true':
                $.smkAlert({text: 'T&eacute;cnico cadastrado com sucesso.', type: 'success', time: 2});
                //window.setTimeout("location.href='autor'", delay);
                $(".form-novo-tecnico-head")[0].reset();
                break;
            ...
            $(".btn-submit-novo-tecnico-head").html('Salvar').fadeTo(fade, 1);
        });
    ...
    $(".form-novo-equipamento").submit(function (e) {
      e.preventDefault();

      $.post("insertEquipamento.php", $(this).serialize(), function (data) {
        $(".btn-submit-novo-equipamento").html('<img src="img/rings.svg" class="loader-svg">').fadeTo(fade, 1);
          ...
          switch (data) {
            ...
            case 'true':
                $.smkAlert({text: 'Equipment successfully registered', type: 'success', time: 2});
                $(".form-novo-equipamento")[0].reset();
                break;
            ...
            $(".btn-submit-novo-equipamento").html('Salvar').fadeTo(fade, 1);
      });
    ...
    $(".delete-equipamento").click(function (e) {
      ...
      $.smkConfirm({
          ...
          // Code here
          if(res) {
              location.href = 'deleteEquipamento.php?' + py + '=' + id;
          }
      });
  });
});
